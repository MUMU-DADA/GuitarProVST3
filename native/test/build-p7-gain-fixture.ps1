...
if (-not $OutputRoot) { $OutputRoot = Join-Path $root '.tools/native/p7-gain-test' }
$bundle = Join-Path $OutputRoot 'P7 Gain Fixture.vst3'
...
& cl /nologo /LD /std:c++17 /EHsc /MD /O2 /utf-8 /D_CRT_SECURE_NO_WARNINGS "/I$QtDir/include" "/I$sdk" @sources "/Fo$OutputRoot/" "/Fe$bin/P7 Gain Fixture.vst3" /link "/LIBPATH:$QtDir/lib" Qt5Core.lib Qt5Gui.lib Qt5Widgets.lib Ole32.lib "/IMPLIB:$OutputRoot/gain.lib"
if ($LASTEXITCODE) { throw 'Gain fixture build failed.' }
@{Name='P7 Gain Fixture';Classes=@(@{CID='10203040506070801122334455667788';Category='Audio Module Class';Name='P7 Gain Fixture';Vendor='Test';'Sub Categories'=@('Fx')})} |
