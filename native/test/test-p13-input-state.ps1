...
if (-not $OutputRoot) { $OutputRoot = Join-Path $root '.tools/native/p13-input-state-test' }
...
$sources = @((Join-Path $root 'native/modules/state_manager.cpp'), (Join-Path $PSScriptRoot 'p13_input_state_test.cpp'))
$exe = Join-Path $OutputRoot 'p13_input_state_test.exe'
& cl /nologo /std:c++17 /EHsc /MD /O2 /utf-8 @include @sources "/Fo$OutputRoot/" "/Fe$exe" /link "/LIBPATH:$(Join-Path $QtDir 'lib')" Qt5Core.lib
if ($LASTEXITCODE) { throw 'P13 input state fixture compilation failed.' }
$env:PATH = "$(Join-Path $QtDir 'bin');$env:PATH"
& $exe
if ($LASTEXITCODE) { throw 'P13 input state regression failed.' }
