...
if (-not $OutputRoot) { $OutputRoot = Join-Path $root '.tools/native/p9-ui-test' }
...
$sources = @((Join-Path $root 'native/modules/state_manager.cpp'),
    (Join-Path $root 'native/modules/qt_ui.cpp'), (Join-Path $PSScriptRoot 'p9_ui_test.cpp'))
$exe = Join-Path $OutputRoot 'p9_ui_test.exe'
...
if ($LASTEXITCODE) { throw 'P9 UI fixture compilation failed.' }
...
try {
    foreach ($scale in @('1', '1.25', '1.5')) {
        ...
        & $exe
        if ($LASTEXITCODE) { throw "P9 UI regression failed at scale $scale." }
    }
} finally { $env:QT_SCALE_FACTOR = $previousScale }
Write-Output "PASS: P9 About, scan feedback and narrow/DPI UI regression. Evidence: $OutputRoot"
