...
if (-not $OutputRoot) { $OutputRoot = Join-Path $root '.tools/native/p8-recognition-test' }
...
$sources = @((Join-Path $root 'native/modules/state_manager.cpp'), (Join-Path $root 'native/modules/vst3_catalog.cpp'), (Join-Path $PSScriptRoot 'p8_recognition_test.cpp'))
$exe = Join-Path $OutputRoot 'p8_recognition_test.exe'
& cl /nologo /std:c++17 /EHsc /MD /O2 /utf-8 @include @sources "/Fo$OutputRoot/" "/Fe$exe" /link "/LIBPATH:$(Join-Path $QtDir 'lib')" Qt5Core.lib
if ($LASTEXITCODE) { throw 'P8 recognition fixture compilation failed.' }
$env:PATH = "$(Join-Path $QtDir 'bin');$env:PATH"
& $exe
if ($LASTEXITCODE) { throw 'P8 recognition regression failed.' }
