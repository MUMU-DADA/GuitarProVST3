...
if (-not $OutputRoot) { $OutputRoot = Join-Path $root '.tools/native/p8-recognition-timeout-test' }
...
if ($LASTEXITCODE) { throw 'P8 recognition timeout fixture compilation failed.' }
$env:PATH = "$(Join-Path $QtDir 'bin');$env:PATH"
& $exe
if ($LASTEXITCODE) { throw 'P8 recognition timeout regression failed.' }
