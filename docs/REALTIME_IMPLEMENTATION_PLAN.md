...
| P13 | 0.10.0 已完成独立 input 链、原生监听贡献分流、SRC/ring 排空和 ASIO 生命周期保护；192000 Hz / 64 帧的延迟、RSE 逐样本共存、三 scope 隔离、尾音/切换、冷启动/设备恢复及发布包门禁通过；其他设备/格式未实测，见实现记录 |
...
- 低延迟输入已在 Studio 2 PRO / Midiplus USB Audio ASIO、实际 192000 Hz / 64 帧、GP 内部 44100 Hz 的固定配置上实测。其他采样率/SRC 拓扑报告 `host_limited`；当前设备拒绝其他 buffer 请求并恢复 64 帧，不能记为这些 buffer 已运行。
- 固定配置物理监听延迟由 78.458 ms 降至 3.177 ms，包含监听路径、DAC、线缆和 ADC；第三方插件延迟与设备差异仍需单独验证。不同 ASIO/WASAPI 设备、其他真实采样率与第三方负载尚无完整矩阵。
