...
- [ ] 使用 GuitarProMCP 通过免安装入口启动原软件，在实际含空格的安装路径中按默认 hook/扫描配置验证：首次按钮提示、重启缓存提速、ParametricOD/Gateway 原生独立窗口、切换/关闭/重开及音频处理。Qt 对象树无法证明窗口显示和布局时使用 MCP 截图补证；不使用 `computer-use`。
