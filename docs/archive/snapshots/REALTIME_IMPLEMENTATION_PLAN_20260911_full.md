...
P7 验收时的 hook 位置是 master 后处理点；P8 已新增并验证真实音轨级 `processDSP` 和独立链，见 [P8 实现记录](P8_IMPLEMENTATION.md)。真实声学听感、设备切换、外部输入监听和每个第三方插件的 editor 兼容性不由该证据推导为完成。
...
## P8：音轨级与全局 VST3 效果器、后台识别和顺序编辑

**状态：P8 六项补齐工作及交付验收已完成（2026-09-11，Guitar Pro 8.1.1.17 / Windows x64）。** 本轮需求见 [P8 计划](P8_TRACK_GLOBAL_VST3_PLAN.md) 与 [P8 实现记录](P8_IMPLEMENTATION.md)。识别隐藏/超时、双音轨真实处理及完整生命周期、原生链保留/编辑、实际分区域列表、窄侧栏、global/track/P4 组合、P7 回归和发布包核对均通过。发布 DLL 可独立完成音轨发现、全轨恢复和实际写回；MCP bridge 是可选 provider。`EffectsChain::index()` 只作诊断，未映射上下文不借用其他音轨的链。
