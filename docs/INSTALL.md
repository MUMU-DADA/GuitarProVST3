...
正常启动后，在音源区域点击 `VST3`。首次扫描时入口立即显示扫描状态；识别结果和“后台识别中”候选项会自动出现。静态扫描完成后由单独控制线程逐个进行 factory 识别，勾选待识别项只记录启用意图，不会阻塞界面；识别成功后自动更新为可启用项，失败保留候选并记录原因。扫描和静态刷新仅读取本地文件，主动识别/启用或恢复插件后，其代码仍可能自行联网。

选择面板提供 `当前音轨` 和 `全局 Master` 两个 scope。已启用项固定显示在各自列表前部，启用区域支持拖动排序，顺序同时写入运行时链和 sidecar；当前宿主尚未提供稳定的 `processDSP` 音轨 ID 时，音轨链保持旁路并在状态中报告 `track_scope_unresolved`。
