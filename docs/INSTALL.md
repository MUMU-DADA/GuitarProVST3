...
音轨 VST3 位于 GP 音轨侧栏的原生音源效果链后；全局 Master VST3 位于曲谱侧栏的母带后期处理后。每次启动都会后台准备识别成功的 VST3 实例并保持旁路，已保存的链仍会显示在列表中，必须再次勾选才会激活处理。勾选项进入“正在使用”，从上到下决定处理顺序，可拖动或使用 `Alt+Up` / `Alt+Down` 排序。切换音轨只改变该轨链，全局链保持独立。

停用只旁路处理并保留实例和参数；启用其他插件、再次启用或调整顺序都复用原实例。音轨移除、关闭曲谱或退出插件时才释放所属实例。完整预加载需要等待后台队列完成，初始化失败的插件会记录错误并继续处理后续插件。

关闭 GUI 或选择区后效果继续工作；勾选切换会先显示“请求中”，后台准备完成后生效，准备期间界面仍可操作。全局插件默认只处理曲谱播放总线，不会自动监听输入设备；需要实时输入监听时显式设置 `GPVST3_P4_ROUTE=input_insert` 或 `bus_mix`。显式开启输入路由后，全局 GUI 的参数编辑同步到独立输入效果，音轨参数保持独立。双击插件名称打开 GUI，停用插件会关闭其 GUI。音轨增删、重排、保存/另存和关闭重开会保留对应链和参数，进程重启后仍需显式重新勾选。
