...
标题工具栏提供“关于”入口；窗口显示版本、已验证宿主范围、许可证、第三方声明和诊断数据目录。若当前宿主没有可识别的标题工具栏，入口会放在菜单栏中。
...
关闭 GUI 或选择区后效果继续工作；勾选切换会先显示“请求中”，由控制线程准备新链并在音频块边界交接，旧链在安全复用前保持可用。停用插件会关闭其 GUI。音轨增删、重排、保存/另存、关闭重开和进程重启会恢复对应链和参数。
