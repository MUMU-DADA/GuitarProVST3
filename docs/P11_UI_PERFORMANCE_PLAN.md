...
状态：代码替换与专项验证已完成；真实宿主功能回归通过，固定设备 CPU/P95 长时 A/B 采样仍需单独执行（2026-09-13）。实施记录见 [P11 实现记录](P11_IMPLEMENTATION.md)。
...
以下各阶段均为未实现；按阶段单独保留差异与证据。P11-2 完成控制职责迁移后，才能永久停用旧 track/observation 驱动。
...
本阶段交付包含运行代码、专项结果和真实 Guitar Pro MCP 双音轨证据；固定设备 CPU/P95 长时 A/B 仍按第 5 节门槛单独采样。“DLL 加载成功”“菜单可见”或单独的 fixture PASS 均不能替代该长时性能证据。
